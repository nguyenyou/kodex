...
# _compare <test_name> <actual_output>
# Common comparison logic shared by run_test and run_help.
_compare() {
    local name="$1"
    local actual="$2"
...
# Runs a kodex command with --idx, captures stdout, compares against snapshot.
run_test() {
    local name="$1"; shift
    ...
    # Capture stdout + stderr (includes disambiguation hints, warnings)
    local actual
    actual=$("$KODEX" "$@" --idx "$IDX" 2>&1) || true
    _compare "$name" "$actual"
}

# run_help <test_name> <subcommand>
# Runs kodex <subcommand> --help (no index needed), compares against snapshot.
run_help() {
    local name="$1"; shift
    ...
    local actual
    actual=$("$KODEX" "$@" --help 2>/dev/null) || true
    _compare "$name" "$actual"
}
