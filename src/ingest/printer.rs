...
pub fn print_info(
    ...
    symtab: &FxHashMap<&str, &proto::SymbolInformation>,
) -> String {
...
struct InfoPrinter<'a> {
    out: &'a mut String,
    symtab: &'a FxHashMap<&'a str, &'a proto::SymbolInformation>,
}
...
impl InfoPrinter<'_> {
    ...
    fn pprint_info(&mut self, info: &proto::SymbolInformation) {
        ...
        Self::write_modifiers_full(self.out, info);
        // Kind keyword
    ...
    fn pprint_signature_with_prefix(
        &mut self,
        sig: &proto::Signature,
        info: &proto::SymbolInformation,
        prefix: &str,
    ...
    fn pprint_signature(&mut self, sig: &proto::Signature, info: &proto::SymbolInformation) {
        ...
        match sv {
            proto::signature::SealedValue::ClassSignature(cs) => {
                ...
                let has_self = cs.self_.as_ref().is_some_and(|t| t.sealed_value.is_some());
                let decl_count = cs
                    .declarations
                    .as_ref()
                    .map_or(0, |d| d.hardlinks.len() + d.symlinks.len());
                if has_self || decl_count > 0 {
                    ...
                    if decl_count > 0 {
                        write!(self.out, "+{decl_count} decls").unwrap();
                    }
                    self.out.push_str(" }");
                }
            }
    ...
    fn pprint_type_normal(&mut self, tpe: &proto::Type) {
        let Some(ref sv) = tpe.sealed_value else {
            return;
        };
        match sv {
    ...
    fn pprint_constant(&mut self, constant: &proto::Constant) {
        let Some(ref sv) = constant.sealed_value else {
            return;
        };
        match sv {
    ...
    fn pprint_defn(&mut self, info: &proto::SymbolInformation) {
        use proto::symbol_information::Kind;
        Self::write_modifiers_defn(self.out, info);
        // Kind keyword (defn style — Scala source keywords)
        match Kind::try_from(info.kind) {
            Ok(Kind::Method | Kind::Constructor) => self.out.push_str("def "),
            Ok(Kind::Macro) => self.out.push_str("macro "),
            Ok(Kind::Type) => self.out.push_str("type "),
            Ok(Kind::Object) => self.out.push_str("object "),
            Ok(Kind::Package) => self.out.push_str("package "),
    ...
    /// Write the display name of a symbol.
    fn write_name(out: &mut String, info: &proto::SymbolInformation) {
        if info.display_name.is_empty() {
            out.push_str("<?>");
        } else {
            out.push_str(&info.display_name);
        }
    }

    /// Write all modifiers for top-level info printing (verbose form).
    fn write_modifiers_full(out: &mut String, info: &proto::SymbolInformation) {
        use crate::model::*;
        let props = info.properties as u32;
        if props & PROP_ABSTRACT != 0 {
            out.push_str("abstract ");
        }
        if props & PROP_FINAL != 0 {
            out.push_str("final ");
        }
        if props & PROP_SEALED != 0 {
            out.push_str("sealed ");
        }
        if props & PROP_IMPLICIT != 0 {
            out.push_str("implicit ");
        }
        if props & PROP_LAZY != 0 {
            out.push_str("lazy ");
        }
        if props & PROP_CASE != 0 {
            out.push_str("case ");
        }
        if props & PROP_COVARIANT != 0 {
            out.push_str("covariant ");
        }
        if props & PROP_CONTRAVARIANT != 0 {
            out.push_str("contravariant ");
        }
        if props & PROP_VAL != 0 {
            out.push_str("val ");
        }
        ...
        if props & PROP_STATIC != 0 {
            out.push_str("static ");
        }
        if props & PROP_PRIMARY != 0 {
            out.push_str("primary ");
        }
        if props & PROP_ENUM != 0 {
            out.push_str("enum ");
        }
        if props & PROP_DEFAULT != 0 {
            out.push_str("default ");
        }
        if props & PROP_GIVEN != 0 {
            out.push_str("given ");
        }
        if props & PROP_INLINE != 0 {
            out.push_str("inline ");
        }
        if props & PROP_OPEN != 0 {
            out.push_str("open ");
        }
        if props & PROP_TRANSPARENT != 0 {
            out.push_str("transparent ");
        }
        if props & PROP_INFIX != 0 {
            out.push_str("infix ");
        }
        if props & PROP_OPAQUE != 0 {
            out.push_str("opaque ");
        }
    }

    /// Write modifiers for definition printing (Scala source keyword style).
    fn write_modifiers_defn(out: &mut String, info: &proto::SymbolInformation) {
        use crate::model::*;
        use proto::symbol_information::Kind;
        let props = info.properties as u32;
        if props & PROP_ABSTRACT != 0 && info.kind == Kind::Class as i32 {
            out.push_str("abstract ");
        }
        if props & PROP_FINAL != 0 && info.kind != Kind::Object as i32 {
            out.push_str("final ");
        }
        if props & PROP_SEALED != 0 {
            out.push_str("sealed ");
    ...
    /// Uses hardlinks if present (borrowed, zero-copy), otherwise looks up
    /// symlinks in the symtab (cloned only when needed).
    fn scope_infos<'s>(&self, scope: &'s proto::Scope) -> std::borrow::Cow<'s, [proto::SymbolInformation]> {
        if !scope.hardlinks.is_empty() {
            return std::borrow::Cow::Borrowed(&scope.hardlinks);
        }
...
fn is_nothing(tpe: &proto::Type) -> bool {
    matches!(
        ...
        Some(proto::r#type::SealedValue::TypeRef(tr))
            if tr.symbol == "scala/Nothing#" && tr.type_arguments.is_empty() && !has_value_opt(tr.prefix.as_deref())
    )
}
...
fn is_any(tpe: &proto::Type) -> bool {
    matches!(
        &tpe.sealed_value,
        Some(proto::r#type::SealedValue::TypeRef(tr))
            if tr.symbol == "scala/Any#" && tr.type_arguments.is_empty() && !has_value_opt(tr.prefix.as_deref())
    )
}
...
fn is_object(tpe: &proto::Type) -> bool {
    matches!(
        &tpe.sealed_value,
        Some(proto::r#type::SealedValue::TypeRef(tr))
            if tr.symbol == "java/lang/Object#" && tr.type_arguments.is_empty() && !has_value_opt(tr.prefix.as_deref())
    )
}

fn has_value_opt(tpe: Option<&proto::Type>) -> bool {
    tpe.is_some_and(has_value)
}
...
mod tests {
    ...
    fn make_type_ref(symbol: &str) -> proto::Type {
        proto::Type {
            sealed_value: Some(proto::r#type::SealedValue::TypeRef(Box::new(
                proto::TypeRef {
                    prefix: None,
    ...
    fn make_value_sig(tpe_symbol: &str) -> proto::Signature {
        proto::Signature {
            sealed_value: Some(proto::signature::SealedValue::ValueSignature(
                proto::ValueSignature {
                    tpe: Some(make_type_ref(tpe_symbol)),
                },
            )),
        }
    }
    ...
    #[test]
    fn val_method_has_colon_separator() {
        // Bug 2: val methods (Method kind + val property + ValueSignature) must have ": "
        // between name and type. Previously output "val method rootDirNameString".
        let info = proto::SymbolInformation {
            kind: proto::symbol_information::Kind::Method as i32,
            properties: 0x400, // val
            display_name: "rootDirName".to_string(),
            signature: Some(make_value_sig("scala/Predef.String#")),
            ..Default::default()
        };
        let symtab = FxHashMap::default();
        let result = print_info(&info, &symtab);
        assert_eq!(result, "val method rootDirName: String");
    }

    #[test]
    fn nullary_method_has_colon_separator() {
        // Nullary methods (def keySpace: KeySpace) also use ValueSignature.
        // Must produce "method keySpace: KeySpace", not "method keySpaceKeySpace".
        let info = proto::SymbolInformation {
            kind: proto::symbol_information::Kind::Method as i32,
            properties: 0,
            display_name: "keySpace".to_string(),
            signature: Some(make_value_sig("com/example/KeySpace#")),
            ..Default::default()
        };
        let symtab = FxHashMap::default();
        let result = print_info(&info, &symtab);
        assert_eq!(result, "method keySpace: KeySpace");
    }

    #[test]
    fn method_with_params_no_double_colon() {
        // Regular methods with MethodSignature should NOT get an extra ": "
        ...
        let info = proto::SymbolInformation {
            ...
            signature: Some(proto::Signature {
                sealed_value: Some(proto::signature::SealedValue::MethodSignature(
                    proto::MethodSignature {
                        type_parameters: None,
                        parameter_lists: vec![],
                        return_type: Some(make_type_ref("scala/Unit#")),
                        throws: vec![],
                    },
                )),
            }),
        ...
        let symtab = FxHashMap::default();
        let result = print_info(&info, &symtab);
        assert_eq!(result, "method process: Unit");
    }

    #[test]
    fn field_still_has_colon() {
        // Fields should still work as before.
        let info = proto::SymbolInformation {
            kind: proto::symbol_information::Kind::Field as i32,
            properties: 0,
            display_name: "count".to_string(),
            signature: Some(make_value_sig("scala/Int#")),
            ..Default::default()
        };
        let symtab = FxHashMap::default();
        let result = print_info(&info, &symtab);
        assert_eq!(result, "field count: Int");
    }
}
